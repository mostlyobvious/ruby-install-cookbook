action :create do
  ...
  archive_name         = "ruby-#{new_resource.name}.tar.bz2"
  ruby_dir             = ::File.join(cache_dir, "ruby-#{new_resource.name}")
  download_source      = "http://rvm.io/binaries/#{node[:platform]}/#{node[:platform_version]}/#{node[:kernel][:machine]}/#{archive_name}"
